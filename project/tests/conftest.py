...
def test_roles(db_session):
    ...
    perms = {
        ...
        "import:stores": Permission(name="import:stores", description="Bulk import stores"),
    }
    ...
    marketer_role.permissions = [perms["read:stores"], perms["write:stores"], perms["delete:stores"], perms["import:stores"]]
...
@pytest.fixture(scope="function")
def tokens(client, test_users):
    """Get all user access tokens in a single fixture to avoid rate limiting"""
    admin_response = client.post("/api/auth/login", json={
        "email": "admin@test.com",
        "password": "AdminTest123!"
    })
    marketer_response = client.post("/api/auth/login", json={
        "email": "marketer@test.com",
        "password": "MarketerTest123!"
    })
    viewer_response = client.post("/api/auth/login", json={
        "email": "viewer@test.com",
    ...
    return {
        ...
        "marketer": marketer_response.json()["access_token"],
        "viewer": viewer_response.json()["access_token"]
    }
...
@pytest.fixture(scope="function")
def marketer_token(tokens):
    """Get marketer access token"""
    return tokens["marketer"]


@pytest.fixture(scope="function")
def viewer_token(tokens):
    """Get viewer access token"""
    return tokens["viewer"]
